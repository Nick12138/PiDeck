...
- 评估桌面端是否需要展示 `summarization_retry_scheduled`、`summarization_retry_attempt_start`、`summarization_retry_finished`。
- `bash_execution_update` 仅在 PiDeck 实际使用 SDK direct RPC bash 时接入。
- 保持 `event-normalize.ts` 白名单；未知事件不得原样透传协议。
- 覆盖 API key、OAuth refresh、environment/header-only auth、custom headers/models、compaction/branch auth。
- 覆盖 retained/background Session 和 Workspace A/B Extension provider 隔离。
- 只有隔离测试证明 provider 泄漏时，才增加 owner/ref-count 基础设施。
